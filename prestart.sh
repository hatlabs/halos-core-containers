...
# Generate Authelia routing configuration
AUTHELIA_ROUTING_FILE="${DYNAMIC_DIR}/authelia-routing.yml"
cat > "${AUTHELIA_ROUTING_FILE}" << EOF
# Authelia routing configuration
# Auto-generated by halos-core-containers prestart
http:
  routers:
    authelia:
      rule: "Host(\`auth.${HALOS_DOMAIN}\`)"
      entrypoints:
        - web
      middlewares:
        - redirect-to-https
...
EOF
chmod 644 "${AUTHELIA_ROUTING_FILE}"
