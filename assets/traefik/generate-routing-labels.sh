...
# Generate localhost redirect configuration
# Redirects http://*.localhost/ to https://*.${HALOS_DOMAIN}/
generate_localhost_redirect() {
    local redirect_file="${MIDDLEWARE_DIR}/localhost-redirect.yml"

    log "Generating localhost redirect to ${HALOS_DOMAIN}"
...
EOF
    ...
    log "Created localhost redirect: ${redirect_file}"
}
...
main() {
    ...
    # Generate localhost redirect (always, regardless of routing files)
    generate_localhost_redirect
